...
static const uint32_t DOUBLE_CLICK_MS = 250;            // ダブルクリック判定窓 (NORMAL モードのみ、短押し応答性重視で短め)
...
static void renderNormal(uint32_t nowMs) {
  ...
  // idle: 予定が無いときは時計 (HH:MM) を控えめな色でスクロール表示
  //   - 動作確認の役目も兼ねる (動き続けることでフリーズしていないことが分かる)
  //   - NTP 未同期時は消灯
  ...
  // 毎フレーム HH:MM を setScrollText に渡す (同一テキストなら no-op)。
  // これにより MANUAL モード復帰直後でも前回の "BREAK" 等が残らず即時切替わる。
  char buf[8];
  snprintf(buf, sizeof(buf), "%02d:%02d", tm.tm_hour, tm.tm_min);
  setScrollText(String(buf));
  ...
  if (nowMs - g_lastScroll >= SCROLL_STEP_MS) {
    g_lastScroll = nowMs;
    disp::drawScrollFrame(g_cols, g_nCols, g_scrollOff, disp::COL_CLOCK);
    if (++g_scrollOff > (int)g_nCols) g_scrollOff = -5;
  }
}
